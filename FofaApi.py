...
class FofaApiClass:
    def __init__(self, code, size, page):
        ...
        try:
            ...
            logs.error('去配置文件把lz的fofa_email 和fofa_key加上')
            sys.exit()
    ...
    def getReqUrl(self):
        return self.url + '?email=' + self.email + '&key=' \
               ...
               + str(self.page) + '&size=' + str(self.size)

    def run(self):
        # print(self.Results)
        if self.Results['error']:
            logs.error(self.Results['errmsg'])
            sys.exit()
        ...
        with open(SaveFileNamePath, 'a+')as f:
            f.writelines(Host)
        logs.success(f'搜索完成 - {self.Results["size"]}条数据 - query:{self.Results["query"]}')